# Benefitline (Good Neighbor track), AWS Agents for Humans hackathon

Benefitline one text line that computes exactly what a family qualifies for with the PolicyEngine rules engine and files it. The full product, demo, and build plan is `PLAN.md` in this folder; read it first and in full. This session owns Benefitline from first line of code to the Devpost submission: code, demo gallery, live deployment, video, README, architecture diagram, MIT license, builder.aws blog post.
